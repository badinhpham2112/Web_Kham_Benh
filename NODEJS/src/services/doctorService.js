...
let checkRequiredFileds = (inputData) => {
    let arrFields = ['doctorId', 'contentHTML', 'contentMarkdowns', 'action', 'selectedPrice',
            ...
            if (checkObj.isValid === false) {
                ...
                if (inputData.action === 'CREATE') {
                    ...
                    if (doctorMardown) {
                        ...
                        doctorMardown.contentMarkdowns = inputData.contentMarkdowns;
        ...
        try {
            if (!inputId) {
                ...
                let data = await db.User.findOne({
                    ...
                    include: [
                        { model: db.Markdown, attributes: ['description', 'contentHTML', 'contentMarkdowns'] },
                        { model: db.Allcode, as: 'positionData', attributes: ['valueEn', 'valueVi'] },
                        {
    ...
    return new Promise(async(resolve, reject) => {
        ...
        try {
            if (!data.arrSchedule || !data.doctorId || !data.date) {
                ...
                console.log('check differenceBy =============================', toCreate);
                //create data
    ...
    return new Promise(async(resolve, reject) => {
        try {
            if (!data.email || !data.doctorId || !data.patientId || !data.timeType) {
                ...
                //send email remed
                await emailService.senAttachments(data);
                resolve({
                    errcode: 0,
                    errMessage: 'ok'
                })
